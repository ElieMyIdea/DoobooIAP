...
function getSkuType(sku: string) {
  if (sku.includes(ITEM_TYPE.CONSUMABLE)) {
    return ITEM_TYPE.CONSUMABLE;
  }
  if (sku.includes(ITEM_TYPE.NON_CONSUMABLE)) {
    return ITEM_TYPE.NON_CONSUMABLE;
  }
  if (sku.includes(ITEM_TYPE.SUBSCRIPTION)) {
    return ITEM_TYPE.SUBSCRIPTION;
  }
  ...
  const renderItem = (item: Product | Subscription): ReactElement => (
    ...
    >
      ...
      >
        ...
        >
          {getSkuType(item.productId) === ITEM_TYPE.SUBSCRIPTION &&
